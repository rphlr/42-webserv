...
void check_file(std::string serverRoot)
{
	std::ofstream file(serverRoot + "/cgi-bin/files.json");
	file << "{\n";
	file << "\t\"files\": [\n";

	std::string path = serverRoot + "/default_webpages/uploads";
...
void HandleRequest::handleRequest(std::string serverRoot) {
	std::string extracted_request = _request.substr(0, _request.find("\n"));
	check_file(serverRoot);
	...
	// handle cookie
	if (_path.find("add_cookie") != std::string::npos) {
		...
		// server.sendCookies(_headers["Cookie"], 1);
	...
	// handle get cookie
	if (_path.find("get_cookie") != std::string::npos) {
